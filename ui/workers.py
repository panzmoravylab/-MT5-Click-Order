...
class ActionWorker(QThread):
    ...
    # action: "buy" | "sell" | "close_symbol" | "close_all"
...
class PollWorker(QThread):
    ...
    ``positions`` nese seznam dictů pozic pro aktuální symbol.
        ...
        while self._running:
            if mt5_service.is_connected():
                ...
                if symbol:
                    ...
                    # Když nemáme symbol, pošleme alespoň celkové pozice? Ne – UI
                    # ukazuje pozice jen pro aktuální symbol. Necháme prázdné.
                    self.tick.emit("", None)
                    self.positions.emit("", [])
            else:
